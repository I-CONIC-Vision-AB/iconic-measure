...
VideoPlayerFrame::VideoPlayerFrame(wxString const &title, boost::shared_ptr<wxVersionInfo> pVersionInfo, int streamNumber, bool bImmediateRefresh, IconicMeasureHandlerPtr pHandler)
    : wxFrame(NULL, wxID_ANY, title),
      ...
      cbIsOpened(false),
      cbFastForward(false),
      cOutputType(OutputStream::EOutputType::NONE),
      cQuality(80),
    cpHandler(pHandler)
{
...
{
        wxString path = wxDirSelector(_("Directory to save image in"));
        if (path.IsEmpty()) {
            return;
        }
        cOutputType = OutputStream::EOutputType::FILE; // Do not create output stream, it is done per image
        cpOutputStream1 = OutputStream::Create(cOutputType, path);
        cOutputFileName.SetPath(path);
    if (cOutputType != OutputStream::EOutputType::NONE) {
        wxInitAllImageHandlers();
//        cQuality = (int)wxGetNumberFromUser(_("Output image quality"), _("JPEG quality factor (0-100)"), _("Save images"), cQuality, 0, 100, this);
    }
}
...
void VideoPlayerFrame::OpenVideo(wxString filename)
{
    ...
    cpImageCanvas = new ImageCanvasHw(this, vAttrs, s.x, s.y, cpDecoder->GetVideoWidth(), cpDecoder->GetVideoHeight(), cpDecoder->UsePbo());
...
void VideoPlayerFrame::GetDecodedFrame()
{
    ...
    if (bFramesDecoded) {
        if (cOutputType != OutputStream::EOutputType::NONE && cpOutputStream1) {
            iconic::gpu::GpuImagePtr pImage = cpImageCanvas->GetImage();
            wxString filename = wxString::Format("frame-%s-%07d.jpg", wxDateTime::Now().FormatISODate(), (int)cpDecoder->GetFrameNumber());
            if (cOutputType == OutputStream::EOutputType::FILE) {
                // Create full path
                cOutputFileName.SetFullName(filename);
                filename = cOutputFileName.GetFullPath();
            }
            switch (cOutputType) {
            case OutputStream::EOutputType::FILE:
            case OutputStream::EOutputType::UDP:
            case OutputStream::EOutputType::FTP: {
                if (!cpThreads) {
                    cpThreads = cpThreads = boost::shared_ptr<boost::thread_group>(new boost::thread_group);
                }
                cpOutputStream1->WriteImage(pImage, *cpThreads, filename);
                break;
            }
            default:
                wxLogVerbose(_("Unhandled output type"));
            }
        }

        gpu::ImagePropertyPtr pProperties = cpDecoder->GetProperties();
        if (pProperties) {
            double lat = pProperties->Get(gpu::ImageProperty::EName::LATITUDE, -512.0);
            if (lat != -512.0) {
                LogStatus("Longitude %.6lf, Latitude %.6lf, Altitude %.3lf, Roll %.6lf, Pitch %.6lf, Yaw %.6lf, Focal length: %.3lf, Principal point: %.3lf, %.3lf",
                            pProperties->Get(gpu::ImageProperty::EName::LONGITUDE, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::LATITUDE, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::ALTITUDE, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::ROLL, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::PITCH, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::YAW, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::FOCAL_LENGTH, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::PRINCIPAL_POINT_X, 0.0),
                            pProperties->Get(gpu::ImageProperty::EName::PRINCIPAL_POINT_Y, 0.0));
                wxFileName fn(cFileName);
                if (!fn.FileExists()) {
                    fn.SetPath(cFileName);
                }
                //wxString frameName = cpDecoder->GetFrameName(cpDecoder->GetFrameNumber() - 1);
                ...
                //fn.SetName(frameName);
                //fn.SetExt("xml");
                //wxString name = fn.GetFullPath();
                //if (!pProperties->Save(name)) {
                //    wxLogError(_("Could not save meta data '%s'"), name);
                //}
            }
        }
        unsigned int const frameNumber = cpDecoder->GetFrameNumber();
        if (frameNumber % 25 == 0) {
            if (wxLog::GetActiveTarget()->GetVerbose()) {
                wxLogVerbose(_("%d frames"), (int)frameNumber);
            }
        }
...
//		cbFastForward = !cbFastForward;
        cbFastForward = false;// (frameNumber % 10) != 0;

        cpDecoder->SetFastForward(cbFastForward);
        cpImageCanvas->SetFastForward(cbFastForward);
        if (cpThreads && cpThreads->size() >= 8) {
            wxLogVerbose(_("Joining threads"));
            cpThreads->join_all();
            wxLogVerbose(_("Threads joined"));
            cpThreads = boost::shared_ptr<boost::thread_group>();
        }
//        threads.join_all();
